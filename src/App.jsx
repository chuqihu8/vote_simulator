...
function PersonFigure({ x, y, propensity, voted, isSelected, isCF, nodeId, onClick }) {
  ...
  return (
    <g transform={`translate(${x},${y})`} onClick={onClick} style={{ cursor: "pointer" }}>
      ...
      {/* ID label below figure — pill backdrop */}
      <rect x="-9" y="27" width="18" height="12" rx="3" fill="white" opacity="0.85" style={{ pointerEvents: "none" }} />
      <text y="33" textAnchor="middle" dominantBaseline="middle" fontSize="9.5" fill="#1e293b" fontWeight="700" fontFamily="'DM Mono',monospace" style={{ pointerEvents: "none" }}>{nodeId}</text>

      {/* Propensity label */}
      <text y="44" textAnchor="middle" fontSize="6" fill={voted ? "#e85d42" : "#475569"} fontWeight="600" fontFamily="'DM Mono',monospace" style={{ pointerEvents: "none" }}>
      ...
      {nodes.map((nd, i) => {
        ...
        return (
          <g key={i} transform={`translate(${pos.x},${pos.y})`}
            ...
            {/* ID label below figure */}
            <rect x="-7" y="24" width="14" height="10" rx="2" fill="white" opacity="0.7" style={{ pointerEvents:"none" }} />
            <text y="30" textAnchor="middle" dominantBaseline="middle" fontSize="8" fill="#1e293b" fontWeight="700" fontFamily="'DM Mono',monospace" style={{ pointerEvents:"none" }}>{i}</text>
            {/* p label */}
            <text y="40" textAnchor="middle" fontSize="5.5" fill="#64748b" fontFamily="'DM Mono',monospace" style={{ pointerEvents:"none" }}>p={nd.propensity.toFixed(2)}</text>
          </g>
        );
      })}
    </svg>
