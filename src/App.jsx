...
export default function App() {
  ...
  const downloadBlob = async (blob, filename) => {
    // Try File System Access API first (most reliable on modern browsers)
  ...
  const runTrial = async () => {
    ...
    try {
      ...
      await downloadBlob(blob, `trial_seed${seed}.zip`);
  ...
  const redownloadTrial = async (entry) => {
    try {
      ...
      const blob = await zip.generateAsync({ type: "blob" });
      await downloadBlob(blob, `trial_seed${entry.seed}.zip`);
      ...
      <header style={{ background: C.white, borderBottom: `1.5px solid ${C.border}`, padding: "10px 18px", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button className="btn btn-green" onClick={async () => {
            ...
            const blob = await zip.generateAsync({ type: "blob" });
            await downloadBlob(blob, `sim_seed${seed}.zip`);
          }}>↓ ZIP</button>
        </div>
      </header>
